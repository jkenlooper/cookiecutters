...
{{ cookiecutter.project_slug_upper }}_PORT="{{ cookiecutter.project_port }}"
{{ cookiecutter.project_slug_upper }}_BIND="127.0.0.1"
...
docker run -i --tty \
  --network=host \
  --env BIND="${{ cookiecutter.project_slug_upper }}_BIND" \
  --env PORT="${{ cookiecutter.project_slug_upper }}_PORT" \
